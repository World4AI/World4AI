<script>
  ...
  export let transitionsWithActions = [
    [
      [0.8, 0.2],
      [0.1, 0.9],
    ],
  ...
  stateCenters.push({
    x: width - stateSize / 2 - paddingLeftRight,
    y: yCenter,
  });
  ...
  actionCenters[0].push({
    x: stateCenters[0].x + actionOffset,
    y: stateCenters[0].y - actionOffset,
  });
  actionCenters[0].push({
    x: stateCenters[0].x + actionOffset,
    y: stateCenters[0].y + actionOffset,
  });
  actionCenters[1].push({
    x: stateCenters[1].x - actionOffset,
    y: stateCenters[1].y - actionOffset,
  });
  actionCenters[1].push({
    x: stateCenters[1].x - actionOffset,
    y: stateCenters[1].y + actionOffset,
    ...
    [
      [
        ...
        [
          ...
          { x: stateCenters[1].x - stateSize / 2, y: stateCenters[1].y - 10 },
        ],
      ],
      [
        [
    ...
    [
      [
        [
          ...
          { x: stateCenters[0].x + stateSize / 2, y: stateCenters[0].y - 10 },
        ],
        [
          {
            x: actionCenters[1][0].x,
...
<svg viewBox="0 0 {width} {height}">
  ...
  {#each points as state, i}
    {#each state as action, a}
      {#each action as coordinates, k}
        <Connection
          d={coordinatesToLine(coordinates)}
  ...
  {#each transitions as transition, i}
    <State
      active={nextState == i}
      x={stateCenters[i].x - stateSize / 2}
      y={stateCenters[i].y - stateSize / 2}
      size={stateSize}
      stateNumber={i}
    />
  {/each}
  ...
  {#if showRewards}
    {#each rewards as reward, i}
      <Reward
        cx={reward.x}
        cy={reward.y}
        degrees={reward.rotate}
        active={i == nextState && nextReward ? true : false}
