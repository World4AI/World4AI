          ...
          arrows = [
            {
              r: observation.r,
              c: observation.c,
              d: actionToDegreeMapping[action],
            },
          ];
...
<svg
  {width}
  height={height + translateGrid}
  version="1.1"
  viewBox="0 0 {width} {height + translateGrid}"
...
>
  {#if showObservation}
    <text
      ...
      fill="var(--text-color)"
      stroke="none">State: ({observation.r}, {observation.c})</text
    >
  {/if}
  {#if showAction}
    <text
      ...
      font-family="sans-serif"
      fill="var(--text-color)"
      stroke="none">Action: ({action})</text
    >
  {/if}
  {#if showReward}
    <text
      ...
      font-family="sans-serif"
      fill="var(--text-color)"
      stroke="none">Reward: ({reward})</text
    >
  {/if}
  ...
  <g id="grid" transform="translate(0, {translateGrid})">
    ...
    {#each cells as cell}
      ...
      >
        <rect
          fill={isColoredReward && cell.reward > 0
            ? "var(--main-color-2)"
            : isColoredReward && cell.reward < 0
            ? "var(--main-color-1"
            : "none"}
          x={cell.c * colSize}
          y={cell.r * rowSize}
      ...
      <g>
        {#each arrows as arrow}
          <line
            ...
            transform="rotate({arrow.d}, {arrow.c * colSize +
