...
<Container>
  ...
  <p>Our overall goal is to generate a sentence (a translation) that maximizes this probability.</p>
  <p><Latex>{String.raw`\arg \max_{w_1,...,w_T} P(w_1, ..., w_T)`}</Latex> </p>
  <p>So how does this work in practice with a recurrent neural network? Let's work through a translation example with a encoder-decoder rnn network.</p>
  ...
  <SvgContainer maxWidth={"1000px"}>
    <svg viewBox="0 0 650 160">
      {#each Array(5) as _, idx}
        <g transform="translate({idx*120 - 20}, 0)">
          ...
          {#if idx >= 2}
            ...
            <Arrow strokeWidth=2 dashed={true} strokeDashArray="4 4" data={[{x: 65, y:20}, {x:110, y: 105}, {x:170, y:105}, {x: 170, y: 98}]} />
            <Block text="Y_{idx+1-2}" fontSize={12} x=65 y=20 width=25 height=15 color="var(--main-color-4)" />
          {/if}
      ...
      {/each}
      <Block x=100 y=22 width=100 height=25 text="Encoder" color=none fontSize={20} />
      <Block x=500 y=140 width=100 height=25 text="Decoder" color=none fontSize={20} />
      <Border x=10 y=50 width=180 height=110 />
      <Border x=250 y=1 width=370 height=110 />
    </svg>
  </SvgContainer>
  <p>As previously discussed the encoder compresses the sentence in the original language into a vecor representation, H_2 in the image above, and passes it to the decoder. The decoder produces one sentence at a time based on the hidden state H and the previously generated word Y. To generate the output Y the hidden state is used as input into a linear layer that generates as many outputs (logits) as there are words in the target language corpus. When we use these logits as an input into a softmax function, we generate a probability distribution, from which we can draw a sample.</p>
  <p>There are many ways to generate samples from a distribution, we are going to use the one that is easiest to impement, called <Highlight>greedy search</Highlight>. Greedy search always samples the word with the highest probability conditioned on the previously produced word Y and the hidden state H. This approach is not optimal, because we are searching for the sentence with the highest probability <Latex>{String.raw`P(w_1, ..., w_T)`}</Latex>. Greedy search on the other hand looks at the highest probability of the next word and not the overall sentence. If we wanted to compare probabilities of full sentences, we would have to create an unlimited amount of sentences. Greedy search is good enough in our case. If you ever want to produce sentences with higher probabilities we recommend you look into <Highlight>beam search</Highlight>. Beam search doesnt't create a single greedy sample, but generates n most likely words at each step. That allows you to compare and contrast those paths and to pick the one with the highest overall probability.</p>
  <div class="separator" />
</Container>
