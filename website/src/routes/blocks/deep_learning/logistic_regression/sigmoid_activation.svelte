...
<Question
  >How can we use an activation function as a probability measure?</Question
>
<div class="separator" />
...
<p>
  ...
  to train a weight <Latex>w</Latex> and a bias <Latex>b</Latex> that produces values
  between 0 and 1. These values can be regarded as probabilities for particular category.
</p>
<Scatterplot
  data={regressionData}
  numTicks={6}
...
/>
<p>
  We could draw a line just like the one above and at first glance this seems to
  be a reasonable approach. Higher values of some feature correspond to a higher
  probability to belong to the "blue" category and lower values of the same
  feature correspond to a lower probability.
</p>
<p>
  ...
  use the same regression line as above but introduce new unseen data our model
  would break apart.
</p>
<Scatterplot
...
<p>
  In our second attempt to construct a classification algorithm we will use the
...
<p>
  ...
  different type of a problem. How do we find the desired weight <Latex>w</Latex
...
/>
<p>
  The outputs <Latex>y</Latex> are always bounded between 0 and 1. This allows us
  to interpret the results probabilities. No matter how large or how negative the
  values are, the output is always between 0 and 1.
</p>
<p>
