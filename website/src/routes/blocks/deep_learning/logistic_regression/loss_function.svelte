...
<h1>Loss Function</h1>
<Question>What loss function is used logistic regression?</Question>
<div class="separator" />
<p>
  As we will see later, the mean squared error is not very well suited to be
  used as the loss function for classification tasks. The loss that we are going
  to use is either called cross-entropy or negative log likelihood. Both names
  are used in the literature and the standard machine learning libraries and
  while the calculation is the same, the motivation (and therefore the
  derivation) is different. To get a better understanding for this type of loss,
  we will talk about both derivations.
</p>
<div class="separator" />
...
<p class="info">The entropy is defined as the expected level of information.</p>
