...
<p>
  ...
  <Latex>{String.raw` MSE=(\hat{y} - y)^2`}</Latex>. Generally speaking we want
  to find the weight vector <Latex>{String.raw`\mathbf{w}`}</Latex> and the bias
  scalar <Latex>b</Latex> that minimize the mean squared error betwen the single
  prediction <Latex>{String.raw`\hat{y}`}</Latex> and the true label <Latex
    >y</Latex
  >.
</p>
<Highlight>
...
<p>
  Just as in the examples above we will use gradient descent to find the minimim
  of the mean squared error. Yet there is a significant difference in the
  notation. What we need to optimize are the weights <Latex
    >{String.raw`\mathbf{w}`}</Latex
  > and the bias<Latex>{String.raw`b`}</Latex> and not the inputs <Latex
    >{String.raw`\mathbf{x}`}</Latex
  >. The vector <Latex>{String.raw`\mathbf{x}`}</Latex> contains <Latex>n</Latex
  >
  features of a single data point and we can not change the features, therefore we
  consider this vector as fixed.
</p>
<p>
...
>
<p>
  This is done for convenience. If you look at the partial derivatives above you
  will notice, that all of them contain the number 2. The division by 2
...
<p>
  ...
  complicated. You should remember from calculus that the derivative of a sum is
  the sum of derivatives. In other words in order to calculate the gradient for
  the MSE below, we need to calculate the individual gradients for each sample,
  add those gradients and divide by the number of samples <Latex>m</Latex>.
</p>
<Highlight>
  <Latex
    >{String.raw`
...
<p>
  Below is the interactive example from the last section. This time we use batch
  gradient descent to find the optimal weight and bias. As we have only 4
  datapoints, batch gradient descent is a fine choice. You can adjust the
...
<p>
  ...
  samples for training. This period of time, in which we exhaust all samples in
  training is called an <Highlight>epoch</Highlight>. After each epoch we
  reshuffle and start over. The advantage of stochastic gradient descent is that
  we do not have to wait for the calculation of gradients for all samples. We
  therefore take a gradient descent step after calculating the gradient for a
  single sample. But we lose the ability of parallelization.
</p>
...
<p>
  ...
  developer. Usually the size is calculated as a power of 2, for example 32, 64,
  128 and so on. You just need to remember that if needs to fit into the memory
