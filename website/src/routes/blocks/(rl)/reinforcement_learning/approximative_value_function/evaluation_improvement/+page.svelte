...
<Container>
  <p>
    Similar to dynamic programming and tabular reinforcement learning we are
    going to apply generalized policy iteration (GPI). In the policy evaluation
    step we are going to look for a value function <Latex
  ...
  <p>
    ...
    in the true value function of that particular policy. Finding the true value
    function is out of the question du to the continuous observation space, so we
  ...
  <p>
    ...
    supervised learning setting and that there is an oracle that tells us what
    the true state-value <Latex>{String.raw`v_{\pi}(s) `}</Latex> for the given policy
    <Latex>\pi</Latex> and state <Latex>s</Latex> is. Later the discussion can be
    extended to reinforcement learning settings where the agent interacts with the
    environment.
  </p>
  <p>
    In supervised learning the goal is to find a weight vector <Latex
      >{String.raw`\mathbf{w}`}</Latex
    > that reduces the error between the true value function <Latex>v(s)</Latex>
    and
    <Latex>{String.raw`\hat{v}(s, \mathbf{w})`}</Latex>. In reinforcement
    learning usually mean squared error (MSE) is used as the measure of error.
  </p>
  <Latex
...
\end {aligned}
  ...
  <p>
    The assumption that there is some oracle that gives us the correct state
    value or action value is obviously not realistic. Therefore we have to
    interact with the environment to generate the approximate value function. As
    per usual the choice is between Monte Carlo and TD learning.
  </p>
  <p>
