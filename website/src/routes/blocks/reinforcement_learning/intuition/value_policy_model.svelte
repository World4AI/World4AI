...
<Question
  >What are the respective components of the agent and the environment?</Question
>
<div class="separator" />
...
<p>
  ...
  the <strong>model</strong>, while the agent might use up to three components:
  the
...
<p>
  ...
  calculate the next state of the environment based on the current state and the
  action chosen by the agent. This process of moving from one state to the next
...
<p>
  ...
  probabilities to transition into the next state and the corresponding reward.
  For example if the state of the environment is 1 and the agent takes action
  number 1, with 50% probability the next state is going to be 1 again and the
  reward will correspond to -1 and with 50% probability the environment will
  transition into the state 2 with reward of 1.
</p>
...
<p>
  ...
  something that the agent has access to. The agent has to learn to navigate in
  an environment where the rules of the game are not known.
</p>
...
<p>
  ...
  agent to work. Nevertheless, the model and the value function are major parts
  of many modern reinforcement learning algorithms. Especially the value
...
<p>
  ...
  action. In more complex environments it is not possible to construct a mapping
...
<p>
  ...
  value function plays an important role in most state of the art reinforcement
  learning algorithms. Intuitively speaking the agent looks at the state of the
  environment and assigns a value of "goodness" to the state. The higher the
...
<p>
  ...
  the goal. The blue value in the bottom left corner is the goal that provides a
  positive reward. The colors inbetween are interpolated based on the distance
  from the goal. The closer the agent is to the goal the higher the values are
  expected to be. Therefore the agent could theoretically look around the
...
<p>
  ...
  Essentially the agent creates some sort of an approximation of the true model
  of the environment. Each interaction allows the agent to improve his knowledge
  regarding the transition probabilities from one state to the next and the
  corresponding rewards. The model can for example be used to improve the
  policy. This is especially useful when interacting with the environment is for
