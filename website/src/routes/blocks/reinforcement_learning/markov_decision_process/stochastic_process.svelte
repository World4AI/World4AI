...
<svelte:head>
    ...
    <meta name="description" content="A Markov decision process is a stochastic process that has the Markov property and provides an interaction mechanism for the agent in form of a decision.">
</svelte:head>
...
<p>The Markov reward process adds a reward for to each state. Whenever a time step passes a reward is generated by a Markov reward process. The reward can theoretically also be calculated stochastically, but for simplicity sake let us assume a deterministic reward.</p>
<p>In a Markov reward process the sequence of random variables not only contains states (reddish tint) but also rewards (white tint).  The process drifts randomly from one state into the next and gives out rewards, but the agent has no influence on the process whatsoever.</p>
<div class="separator"></div>
<h2>Markov Decision Process</h2>
<p>A Markov decision process adds, as the name suggests, decisions to the Markov chain. In other words the agent gets some agency over it's decision.</p>
<p class="info">A Markov decision process is a Markov chain, where a part of the chain is influenced by the decisions of the agent.</p>
<Mdp showRewards={true} showActions={true} />
<p>In the example above the environment has still two states, state 0 and state 1 (represented by red boxes). Landing in 0's state is more advantagous as the agent receives a reward of 5, while landing in the 1's state generates a reward of -1 (white circles). In both states the agent can choose between two actions (blue circles). The 0's action is the action toward the 0's state and the 1's action is the action towards the 1's state. For example if the agent is in the 0's state it will choose the action to move (or better remain) left with a probability of 30% and right with a probability of 70%. If it chooses the 1's action the environment will transition towards the 0's state in 10% of cases and towards the 1's state with 90% of cases. The whole process is the succession of states, actions and rewards. This is the same mechanism, that is more often introduced as the interaction between the agent and the environment. The interaction is nothing more than a Markov chain that is made out of two elements. The first element, the environment is static and can not be changed in any form. The second component is the agent. The agent can change the probabilities of the whole chain, by tweaking the probabilities of actions. In the example above, when the agent starts improving we expect it to increase the probabilities to move to state 0. Ideally that would mean that the agent will only choose action 0.</p>
<div class="separator"></div>
<style>
    .text-style {
