...
<p>
  ...
  a tuple <Math latex={"(\\mathcal{S, A}, P, R, \\gamma)"} />, where each
  individual component of that tuple is a required component of a valid MDP. The
  definition of each component is going to be important in many subsequent
  sections, because those definitions are the basis of most mathematical proofs
...
<p>
  In a Markov decision process <Math latex={"\\mathcal{S}"} /> is the state space,
  that contains all possible states of the environment.
</p>
...
<p>
  ...
  looks as follows: <Math
    latex={"\\mathcal{S}=[(0,0), (0, 1), (0, 2), ... , (4, 4)]"}
  />.
</p>
<p class="info">
  <Math latex={"\\mathcal{S}"} /> is the set of all legal states.
</p>
<div class="separator" />

<h2><Math latex={"\\mathcal{A}"} />: Actions</h2>
<p>
  In a Markov decision process <Math latex={"\\mathcal{A}"} /> is the action space,
  that contains all possible actions of the environment.
</p>
<div class="flex-center">
...
<p>
  ...
  action space in that particular grid world is <Math
    latex={"\\mathcal{A}=[0, 1, 2, 3]"}
  />. Even if in some states it is not possible to move into a particular
  direction, the state space is usually kept consistent across the whole state
  space.
</p>
<p class="info">
  <Math latex={"\\mathcal{A}"} /> is the set of all legal actions.
</p>
<div class="separator" />

<h2><Math latex={"P"} />: Transitions</h2>
<p>
  Each Markov decision process possesses a transition probabilities function <Math
    latex={"P"}
  /> that provides a probability for the next state <Math latex={"s'"} /> given the
  current state <Math latex={"s"} /> and the action <Math latex={"a"} />.
  Mathematically we can express that idea as follows:
</p>
<Math
  latex={"P(s' \\mid s, a) \\doteq Pr[S_{t+1}=s' \\mid S_t=s, A_t=a], \\forall s, s' \\in \\mathcal{S}, a \\in \\mathcal{A}"}
/>
<div class="flex-center">
  <Grid env={env_3} />
</div>
<p>
  ...
  decides to move to the east, therefore <Math latex={"s=(0,0), a=1"} />. Let us
  ...
  1/3 of cases to the right of the desired direction. In the initial state of <Math
    latex={"s=(0,0)"}
...
<p>
  The reward function calculates the expected value of the reward given state <Math
    latex={"s"}
  /> and action <Math latex={"a"} /> at time step <Math latex={"t"} />.
...
<p>
  In the example above the state of the environment is <Math
    latex={"(3, 0)"}
  />. The agent selects the action 2 to move south and to hopefully receive a
  positive reward. If the agent lands on the blue square the game ends and the
  agent receives a reward of 1. In any other case the agent lands on the red
  square and receives a reward of -1. Just as in the example above the state
  transitions only in 1/3 of cases in the desired direction. When the agent
  selects the action 2 there is a 33.3% chance of landing on the triangle, a
  33.3% chance of not moving and a 33.3% chance of moving to the right. The
  given state and action imply a reward of approximately -0.33.
</p>
<Math
  latex={"r((3, 0),2) = \\mathbb{E}[R_{t+1} \\mid S_{t}=(3,0),A_{t}=2] = 0.33 * (-1) + 0.33 * (-1) + 0.33 * (+1) = -0.33 "}
/>
...
<p>
  ...
  should be a compensation if the agent decides to delay his reward. The gamma <Math
    latex={"\\gamma"}
...
<p class="info">
  <Math latex={"\\gamma"} /> (gamma) is the discount factor, where <Math
    latex={`0 \\leq \\gamma \\leq 1`}
  />.
</p>
<p>
  The value of rewards from the perspective of the agent at time step <Math
    latex={"t"}
  /> is as following:
</p>
<p>
  The value of a reward received at timestep <Math latex={"t+1"} /> is <Math
    latex={"\\gamma^0 * R_{t+1}"}
  />
</p>
<p>
  The value of a reward received at timestep <Math latex={"t+2"} /> is <Math
    latex={"\\gamma^1 * R_{t+2}"}
  />
</p>
<p>
  The value of a reward received at timestep <Math latex={"t+3"} /> is <Math
    latex={"\\gamma^2 * R_{t+3}"}
  />
</p>
<p>
...
<p class="info">
  ...
  state in an episodic task is called a <em>terminal state</em>. The letter <Math
    latex={"T"}
...
<p>
  At <Math latex={"t+3"} />: the discount factor is <Math
    latex={"\\gamma^2 = 0.81"}
  />
</p>
<p>
  At <Math latex={"t+5"} />: the discount factor is <Math
    latex={"\\gamma^4 = 0.66"}
  />
</p>
<p>
  At <Math latex={"t+11"} />: the discount factor is <Math
    latex={"\\gamma^{10} = 0.35"}
  />
</p>
<p>
  At <Math latex={"t+21"} />: the discount factor is <Math
    latex={"\\gamma^{20} = 0.12"}
  />
</p>
<p>
  At <Math latex={"t+51"} />: the discount factor is <Math
    latex={"\\gamma^{50} = 0.005"}
  />
</p>
<p>
...
<p class="info">
  Gamma <Math latex={"\\gamma"} /> is defined as a part of an MDP, but in practice
  it is treated as a hyperparameter of the agent. Theoretically gamma should be obvious
  from the environment. In practice there is no clear indiciation regarding how large
  <Math latex={"\\gamma"} /> should be. Tweaking the value might produce better results,
  because gamma can have an impact on how fast the algorithm converges and how stable
