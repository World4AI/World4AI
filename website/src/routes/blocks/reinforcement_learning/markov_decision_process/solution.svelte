...
<p>
  ...
  simply the sum of rewards starting from timestep <Latex>t+1</Latex> and going either
  to terminal state <Latex>T</Latex> (episodic tasks) or to infinity (continuing
...
<p>
  ...
  terminal state <Latex>T</Latex>. This sequence of states, actions and rewards
  of an episode is called a <em>trajectory</em>.
</p>
<Trajectory />
<p>
...
<p>
  ...
  > is extended to all other types of rewards. Specificylly in reinforcement learning
  we use the geometric series to calculate the current value of a future reward.
</p>
...
<p>
  ...
  example at each timestep the agent receives a reward of +1 and the episode has
  no natural ending, the return <Latex>G</Latex> could theoretically become infinite.
  Due to the geometric progression of gamma (<Latex>\gamma</Latex>) the
  discounted returns approach 0 when they are far into the future.
</p>
<p>
  The interactive example below show how the discounting rate progresses towards
  0 (y-axis) depending on timesteps (x-axis) and the gamma. The lower the gamma
  the faster the progression.
</p>
<Discounting />
<div class="separator" />
...
<h2>Policy</h2>
<p>
  A policy is a mapping from states to either actions directly or a probability
  of an action, depending on whether the policy is deterministic or stochastic.
</p>
<p>
...
<p>
  ...
  actions. To generate an action using a stochastic policy we define <Latex
...
<p>
  ...
  stochastic policies. That stochasticity produces different trajectories and
  therefore different returns <Latex>G_t</Latex>
  even when the starting state <Latex>S_0</Latex> and the policy <Latex
    >{String.raw`\pi`}</Latex
  >
  remain the same. But how can we measure how good it is for the agent to use a certain
  policy <Latex>\pi</Latex>, if the generated returns are not consistent? By
  using value functions!
</p>
<p>
  For a given policy <Latex>\pi</Latex> value functions map states or state-action
  pairs to “goodness” values, where goodness is calculated as the expected sum of
  rewards and higher values mean more favorable states or state-action pairs.
</p>
<p class="info">
  In simple words value functions calculate the following: How much return <Latex
    >G_t</Latex
  > does the agent expect to receive when it faces the state <Latex>S_t</Latex> and
  always follows the policy <Latex>\pi</Latex>.
</p>
<p>
...
<p>
  ...
  the expected return when the agent faces the state <Latex>s</Latex>, takes the
...
<p>
  ...
  the state set <Latex>{String.raw`\mathcal{S}`}</Latex>.
</p>
<p>
...
<p>
  ...
  we play for an infinite amount of timesteps before we can calculate a single return.
  Bellman equations on the other hand will allow us to work with estimates and to
  use a return from a single timestep to make improvements.
</p>
<p>
