...
<Question
  >How can we generalize the calculation of the advantage function to use in
  reinforcement leanring?</Question
>
<div class="separator" />
...
<p>
  ...
  but in modern reinforcement learning algorithms it is most likely to contain an
  advantage estimation <Latex>A(S_t, A_t)</Latex>, making it an advantage
...
<p>
  ...
  up with a one step temporal difference advantage estimation or n is unbounded
  and we end up with a full monte carlo estimation. In the A3C algorithm n
  corresponded to 5, but it is not clear if it was the right choice.
</p>
<p>
  ...
  <Latex>n</Latex>, to hopefully end up with a more robust advantage estimation.
  To come up with a better advantage estimator, each of the <Latex>n</Latex> estimators
