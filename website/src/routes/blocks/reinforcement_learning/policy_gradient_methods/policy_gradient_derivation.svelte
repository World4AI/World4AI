...
<p>
  ...
  more intuitive that any other derivation we have encountered and is therefore
...
<p>
  ...
  >. Each of the trajectories has a corresponding return <Latex>G</Latex>. When
  we talk about policy gradients, the return is defined as <Latex>R(\tau)</Latex
  > to indicate that the return is depends on the trajectory that was unrolled throught
  the interaction.
</p>

<p>
  The general goal of a reinforcement learning agent is to maximize the expected
  return. In policy gradient methods the expected return is also called the <Highlight
    >objective function</Highlight
  >
  <Latex>J</Latex>.
</p>
...
<p>
  ...
  in a later step. The likelihood ratio trick utilizes the following identity.
</p>
<Latex
  >{String.raw`
...
<p>
  ...
  and on the other hand the model of the Markov decision process <Latex>P</Latex
