...
<p>
  ...
  amount of variance. In this chapter we introduce an improvement to REINFORCE.
  The algorithm is called REINFORCE with baseline, but sometimes the name
  "vanilla policy gradient" (VPG) is also used.
</p>
...
<p>
  ...
  probability of actions that generate negative returns is decreased. But what
  if all returns are positive or clustered? The probability of actions with
  highest returns will increase more than those with lower returns. But the
  process is slow.
</p>
<p>
