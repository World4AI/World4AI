...
<p>
  ...
  timestep the agent takes the agent receives a reward of -1. If the agent lands
  in the terminal state 0, the agent receives a reward of -10. To simplify the
  coming calculations we are going to let the agent take up to 2 actions. After
  the 2 actions the game will restart.
</p>
<!-- Show the grid world -->
...
<p>
  ...
  direction, where 0 is the left and 1 is the right action. In 20% of cases the
  state does not change. Once the agent reaches state 0 or 1, the reward is
  exhausted and the agent can not leave the terminal state.
</p>
<!-- Show model-->
<Table header={modelHeader} data={modelData} />
<p>
  The optimal policy is obviously to keep moving right, but we initialize the
  policy of the agent with random behaviour in order to demonstrate the learning
  process.
</p>
<!-- Show policy -->
<Table header={policyHeader} data={policyData} />
<p>
  Below is the tree representing all paths that can be reached after two actions
  from state 1. The larger circles are the states with the corresponding
  rewards. Below those circles you can see a mini map, where the highlighted
  square indicates the location of the agent. The smaller cirlces are the
  possible actions, where the left circles represent the movement to the left
  and vice versa. The two components that you should notice are the red and blue
...
<p>
  ...
  probabilities to go left and right at the top node in the tree below? The left
  path produces an expected value of -4.56 and the right path produces the
  expected value of 0.4. If we change the probability to go into a particular
  direction proportional to the expected values, then we would automatically
  increase the probability to move right and decrease the probability to move
  left.
</p>
...
<p>
  ...
  left is -0.4 and of going right is 1.36, we therefore increase the probability
  of going right and decrease the probability of going left.
</p>
<MDPTree root={markedTree2} />
<!-- Show tree with sliders -->
<p>
  Below is the tree with the same initial conditions. We provided two sliders,
  that allow you to tweak the probabilities to go left and right in state 1 and
  2. When you move the sliders to the right you increase the probability to go
  right and because the probabilities sum up to 1, you simultaneously decrease
  the probability of going to the left. You should also observe that state 1 is
  encountered several times, therefore there is a lot of interaction by tweaking
  the above slider.
</p>
<MDPTree root={tree} />
...
<p>
  ...
  you can sample the different paths, as indicated in the animation below. After
  you collected some samples you can scale the probability of actions by the
  returns of those samples. Some individual draws might change the probability
  into the wrong direction but in expectation we should find the optimal policy.
</p>
...
<p>
  ...
  framework like PyTorch or TensorFlow to calculate the gradient of the policy.
  That way we can figure out how we should tweak the weights to increase the
  probability of an action. And finally we scale the gradient by sampled
  returns. Larger returns increase the probabilities more than lower returns (or
  negative returns that decrease the probabilities). Because the probabilities
  sum to 1 good paths should become more likely to be drawn and bad paths will
  become unlikely.
</p>
<div class="separator" />
