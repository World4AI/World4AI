...
<p>
  with torch.no_grad(): target = rewards + self.gamma *
  self.Q_target(next_obss).max(dim=1, keepdim=True)[0] * (1 - dones)
</p>
<p>
  with torch.no_grad(): next_actions = self.Q(next_obss).max(dim=1,
  keepdim=True)[1].long() target = rewards + self.gamma *
  self.Q_target(next_obss).gather(dim=1, index=next_actions) * (1 - dones) `}
</p>
