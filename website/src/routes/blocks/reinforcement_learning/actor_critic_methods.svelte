...
<Question>What are Actor-Critic methods?</Question>
<div class="separator" />
...
<p>
  ...
  so-called actor-critic methods. The actor is the decision maker and the policy <Latex
    >\pi</Latex
  >
  ...
  functions as neural networks and train them simultaneously. The actor-critic methods
  can have significant improvements over pure value or policy based methods and in
  many cases constitute state of the art methods.
</p>
<p>
...
>
<p>
  REINFORCE with baseline has a policy <Latex>{String.raw`\pi_{\theta}`}</Latex>
...
<p>
  ...
  <Latex>{String.raw`V(S_{t + 1})`}</Latex> we ask the critic to calculate the expected
  value of the next state<Latex>{String.raw`S_{t+1}`}</Latex> that resulted from
  the actor taking an action <Latex>{String.raw`A_t`}</Latex>. That way the
  critic essentially "critiques" the previous action of the actor.
</p>
<p>
  In the following sections we will discuss some of the actor-critic algorithms.
  Especially we will focus on so called advantage actor-critic methods.
</p>
<div class="separator" />
