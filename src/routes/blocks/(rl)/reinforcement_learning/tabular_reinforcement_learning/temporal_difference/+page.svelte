...
<Container>
  <p class="info">
    Whereas conventional prediction-learning methods assign credit by means of
    the difference between predicted and actual outcomes, the new methods assign
    credit by means of the difference between temporally successive predictions.
  </p>
  <p>
    Instead of waiting for the end of an episode to use the difference between
    the predicted state or action value and the actual return <Latex>G_t</Latex>
    temporal difference methods calculate the difference between the value of the
    current state <Latex>s</Latex> and the value of the next state <Latex
      >s'</Latex
    >.
  </p>
  ...
  <p>
    ...
    update steps. As we do not have access to actual values for the next state
    we have to use estimates for target values, we need to use bootstrap. When
    the agent uses temporal difference methods the agent does not have to wait
    for the end of the episode to apply an update to the estimate of the value
    function, but can instead update the estimation function after a single
    step.
  </p>
  <p>
    Temporal difference learning is also based on generalized policy iteration,
    which alternates between policy evaluation and policy improvement to arrive
  ...
  >
  <p>
    Using the same logic the update step can be adjusted to reflect the
    recursive nature of the value function by using bootstrapping.
  </p>
  <Latex
  ...
  >
  <p>
    The quantity <Latex>{String.raw`V(S_{t+1}) - V(S_t)`}</Latex> is the temporal
    difference: difference of values measured at successive states. The quantity
    <Latex>{String.raw`R_{t+1} + \gamma V(S_{t+1}) - V(S_t)`}</Latex> is called TD
    error. TD error the difference between the bootstrapped target and the estimated
    value. The larger the error
    <Latex>\delta</Latex>, the larger the adjustment that is done to the
    estimate.
  </p>
  <p>
  ...
  <p>
    ...
    cleaner. The interaction step and the update step are consolidated and do
    not have to be put into different loops or functions.
  </p>
  ...
  <p>
    ...
    generate actions (<Latex>\epsilon</Latex>-greedy in our case) is also the
    one that is being improved. At each timestep the agent improves the
    action-value function, by using a bootstrapped version of the target.
  </p>
  <Latex
    >{String.raw`Q(S_t, A_t) \leftarrow Q(S_t, A_t) + \alpha [R_{t+1} + \gamma Q(S_{t+1}, A_{t+1}) - Q(S_t, A_t)]`}</Latex
  >
  <p>
    Both actions <Latex>A_t</Latex> and <Latex>{String.raw`A_{t+1}`}</Latex> are
    picked using the <Latex>\epsilon</Latex>-greedy policy <Latex
      >\pi(a \mid s)</Latex
    >.
  </p>
  <p>
  ...
  <p>
    Q-Learning is the off-policy TD control algorithm. In this algorithm we use <Latex
      >\epsilon</Latex
    >-greedy to take actions, yet a purely greedy strategy is the one that we
    aim to improve.
  </p>
  <Latex
