...
<Container>
  ...
  <h3>Asynchronity</h3>
  <p>
    Policy gradient methods and by extension actor-critic algorithms are
    on-policy algorithms. To see why this is the case let us look at the
    calculation of gradient that is used in the gradient ascent step.
  </p>
  <Latex
    >{String.raw`
  ...
  >
  <p>
    In order to calculate the gradient, the expectation has to be estimated
    based on trajectories that are sampled with the help of the policy <Latex
    ...
    the trajectories are drawn from a distribution that depends on the policy <Latex
      >\pi</Latex
    >, which in turn depends on the neural network parameters <Latex
      >\theta</Latex
  ...
  <p>
    ...
    learning: the highly correlated observations. The experience replay
    techniques draws random experiences from the memory buffer. This randomness
    increases the likelihood, that tuples from different episodes are drawn,
    which reduces the correlation between the observations.
  </p>
  <p>
  ...
  <p>
    ...
    running the same algorithm in parallel on different processor cores. Each
    core has a distinct agent that interacts with its own instance of the
    environment and updates the policy periodically. It is reasonable to assume
    that each agent faces different states and rewards when interacting with the
    ...
    <svg version="1.1" viewBox="0 0 500 500" xmlns="http://www.w3.org/2000/svg">
      ...
      <g stroke="var(--text-color)">
        ...
        >
          ...
          <path
            d="m89.537 303.17h16.655v16.708h-16.655v-16.708"
            fill="#ff8080"
          />
          ...
          <path
            d="m89.537 319.88h16.655v16.708h-16.655v-16.708"
            fill="#ff8080"
          />
          ...
          <path
            d="m122.85 336.58h16.655v16.708h-16.655v-16.708"
            fill="#ff8080"
          />
          ...
          <path
            d="m56.228 436.83h-16.655v-16.708h16.655v16.708"
            fill="#ff8080"
          />
          ...
          <path
            d="m56.228 420.12h-16.655v-16.708h16.655v16.708"
            fill="#ff8080"
          />
          ...
          <path
            d="m22.919 403.42h-16.655v-16.708h16.655v16.708"
            fill="#ff8080"
          />
  ...
  <p>
    ...
    presumably different cores. Each agent that is spawned on a different
    process copies the shared weights into the local functions before each
    interaction sequence. After a certain number of steps or when the agent
    encounters the terminal state, the agent runs the gradient descent step on
    the global shared weights and copies the global shared weights into the
    local policy and value functions. The copying and updating is done in an
    asynchronous, non blocking, way. That means that there is no communication
    and coordination of individual agents. Sometimes the gradients that are
    copied back into the global shared value and policy network might be
    overridden by a different agent. This algorithm, called Hogwild, still
    manages to work well, even when the agents that live on different processes
    do not communicate with each other.
  </p>
  <p>
    The A3C algorithm uses different cores of the CPU to spawn different agents
    and environments. The experience batches are small and there is no need to
    use any GPUs, therefore the forward and backward passes are conducted
    exclusively on the CPU.
  </p>
  <h3>Advantage</h3>
  <p>
    The A3C algorithm uses n-steps to calculate the advantage that is used in
    the calculation of the gradient. If the environment terminates before the
    n'th step the optimization is done with the data that is available. In the
    original paper the authors use 5 steps. The rest of the trajectory return is
    calculated through bootstrapping, which makes the algorithm an actor-critic
    algorithm. Below we can see the calculation of the advantage for each of the
    5 steps.
  </p>
  <Latex
    >{String.raw`
  ...
  <p>
    ...
    distribution. The more certain the actions become the lower the entropy gets.
    If we assume for example a policy with two actions, then a policy which tends
    to select the same action for a given state with almost 100% probability will
    have a very low entropy. If on the other hand both actions will be selected with
    50% probability for a given state, then the entropy will be high.
  </p>
  <p>
    <Latex>H</Latex> calculates the entropy and beta <Latex>\beta</Latex> is used
    to measure the importance of the entropy. The reason for using entropy is to
    encourage exploration. The general idea of the policy gradient algorithm is to
    maximize actions with the highest advantage, but if the convergence to certain
    actions happen too soon, it is possible that the agent misses on more favorable
    actions. Higher entropy forces the policy function to contain more uncertainty
  ...
  <p>
    The improvements to performance that were made with the asynchronous
    advantage actor critic (A3C) were not necessarily due to the asynchronous
    aspect of the algorithm. Soon after the release of the algorithm researchers
    found out that the performance boost comes from the experiences that are
    generated by different environments and not the asynchronous updates that
    were made with Hogwild. In A2C (advantage actor critic) several environments
    are generated and a batch of experiences is gathered. But the update is done
    centrally by a coordinating entity. The advantage of this approach is the
    easier implementation and debugging, while keeping the decorelated
    experience tuples that result from the many distinct environments.
    Additionally as the update is done with experiences that can be packed into
    a batch, GPUs can be heavily used with A2C to improve the performance.
  </p>
  <p>
    There is no single paper that can be pointed to for the A2C algorithm. But
    the name has become popular after OpenAI released its baseline
    implementations for most common state of the art reinforcement learning
    algorithms.
  </p>
  <div class="separator" />
</Container>
