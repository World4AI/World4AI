...
<Container>
  <p>
    Proximal policy optimization is the direct successor to TRPO, where we try
    to optimize the following surrogate objective.
  </p>
  <Latex
  ...
  <p>
    ...
    in the optimization. We would need to utilize several mathematical tricks and
    can generally not easily implement TRPO utilizing existing "automatic differentiation"
    libraries like PyTorch or TensorFlow.
  </p>
  <p>
    The PPO paper introduces a clipped objective function, that tries to solve
    the same problem, but in a much easier manner.
  </p>
  <Latex
  ...
  <p>
    In the first step we clip the ratio between the old policy and the new
    policy between <Latex>1-\epsilon</Latex> and
  ...
  <p>
    ...
    understand why this is necessary we need to consider two cases: the
    advantage <Latex>{String.raw`\hat{A}`}</Latex> is posivite and the advantage
    ...
    When <Latex>{String.raw`\hat{A}`}</Latex> is negative, gradient descent will
    attemt to push the ratio as low as possible, but the min operation will make
    sure that the ratio is not lower than <Latex>1-\epsilon</Latex>, because a
    lower ratio between the two probabilities generates a higher number when the
    advantage is negative. For example <Latex>0.9 * (-1) \lt 0.8 *(-1)</Latex>.
  </p>
  <p>
