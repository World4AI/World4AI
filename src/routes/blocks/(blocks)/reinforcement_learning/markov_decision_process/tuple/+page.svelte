...
<Container>
  <p>
    ...
    >, where each individual component of that tuple is a required component of
    a valid MDP. The definition of each component is going to be important in
    many subsequent sections, because those definitions are the basis of most
  ...
  <p>
    In a Markov decision process <Latex>{String.raw`\mathcal{S}`}</Latex> is the
    state space: the set that contains all possible states of the environment.
  </p>
  ...
  <p>
    In a Markov decision process <Latex>{String.raw`\mathcal{A}`}</Latex> is the
    action space, that contains all possible actions of the environment.
  </p>
</Container>
<div class="separator" />
...
<Container>
  ...
  <p>
    ...
    can express that idea as follows:
  </p>
  <Latex
  ...
  <p>
    ...
    possible states and 2 possible actions. Pick the current state-action pair
    and observe how the distribution of the next states changes.
  </p>
  <div class="separator" />
