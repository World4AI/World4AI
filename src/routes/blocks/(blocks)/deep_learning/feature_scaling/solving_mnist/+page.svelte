...
<Container>
  <p>
    It is tradition in the deep learning community to kick off the deep learning
    journey, by classifying handwritten digits using the <a
      href="http://yann.lecun.com/exdb/mnist/"
      rel="noreferrer"
      target="_blank">MNIST</a
    > dataset.
  </p>
  <p>
    Additionally to the libraries, that we have used before we will utilize the <a
      href="https://pytorch.org/vision/stable/index.html"
      target="_blank"
      rel="noreferrer">torchvision</a
    > library. Torchvision is a part of the PyTorch stack that has a lot of useful
  ...
  <p>
    The torchvision <code>MNIST</code> class downloads the data and returns a
    <code>Dataset</code> object.
  </p>
  <PythonCode code={code2} />
  <p>
    The <code>root</code> attribute designates the folder where the data will be
    kept. The <code>train</code> property is a boolean value. If True the object
    returns the train dataset, if False the object returns the test dataset. The
    <code>download</code> property is a boolean, which designates whether the data
    should be downloaded or not. You usually need to download the data only once,
    after that it will be cached in your root folder.
  </p>
  <p>
    ...
    first neural network on the other hand, MNIST is the perfect dataset.
  </p>
  ...
  <p>
    When we look at the minimum and maximum pixel values, we will notice that
...
Maximum pixel value: 255
  ...
  <p>
    This is the usual range that all images have. The higher the value, the
    ...
    stands for batch size, channels, hight, width.
  </p>
  <p>
    When it comes to computer vision, PyTorch provides scaling capabilities out
    of the box in <code>torchvision.transforms</code>.
  </p>
  <PythonCode code={code6} />
  <p>
    When we create a dataset using the <code>MNIST</code> class, we can pass a
    <code>transform</code>
    ...
    <a
      ...
      target="_blank"
      rel="noreferrer">Torchvision Docs</a
    >, but sometimes you might want more control. For that purpose you can use
    <code>transforms.Lambda()</code>, which takes a Python lambda function, in
    ...
    <code>transform.Compose([transform1,transform2,...])</code>. Below we
    prepare two sets of transforms. One set contains feature scaling, the other
    does not. We will both apply to MNIST and compare the results.
  </p>
  <p>
    The first set of transforms first transforms the PIL image into a Tensor and
  ...
  <p>
    Those transforms do not include any form of scaling, therefore we expect the
    training to be relatively slow.
  </p>
  <PythonCode code={code8} />
  <p>
    Below we calculate the mean and the standard deviation of the images pixel
    ...
    have 3 channels and would therefore require 3 mean and std calculations.
  </p>
  <PythonCode code={code9} />
  <p>
    The second set of transforms first applies <code>transforms.ToTensor</code>
    which turns the PIL image into a float32 Tensor and scales the image into a
    0-1 range. The <code>transforms.Normalize</code> transform conducts what we call
    ...
    3 channels, you need to provide a tuple of mean and std values, 1 for each channel.
  </p>
  <PythonCode code={code10} />
  <PythonCode code={code11} />
  <PythonCode code={code12} />
  <p>
    Based on the datasets we create two dataloaders: <code>dataloader_orig</code
    >
    without scaling and <code>dataloader_normalized</code> with scaling.
  </p>
  <PythonCode code={code13} />
  <PythonCode code={code14} />
  <p>
    The <code>train</code> function is the same generic function that we used in
    the previous PyTorch tutorials.
  </p>
  <PythonCode code={code15} />
  <p>
    The <code>Model</code> class is slighly different. Our batch has the shape
    ...
    rows of the image. The layer <code>nn.Flatten()</code> does just that. Our output
    ...
    because our loss function will combine the softmax with the cross-entropy loss.
  </p>
  <PythonCode code={code16} />
  <p>
    Below we train the same model with and without feature scaling and compare
    the results. The <code>CrossEntropyLoss</code> criterion stacks the log
    ...
    <code>nn.LogSoftmax</code>
    activation to your model and use the <code>nn.NLLLoss</code>, but that is
    not recommended.
  </p>
...
Epoch: 10 Loss: 0.047709111124277115
  ...
  <p>
    The difference is huge. Without feature scaling training is slow and the
    loss oscilates from time to time. Training with feature scaling on the other
    hand decreases the loss dramatically.
  </p>
  <Alert type="warning">
    Not scaling input features is one of the many pitfalls you will encounter
    when you will work on your own projects. If you do not observe any progress,
    check if you have correctly scaled your input features.
  </Alert>
  <div class="separator" />
</Container>
