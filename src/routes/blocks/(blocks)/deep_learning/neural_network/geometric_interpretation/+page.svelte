<script>
  ...
  const layers = [
    ...
    {
      title: "Output",
      nodes: [{ value: "o", class: "fill-blue-400" }],
    },
  ];
...
<Container>
  <p>
    So far we have discussed how a computational graph works and how we can use
    ...
    networks. We need to answer the following question. <Highlight
      >How does a solution to a nonlinear problem looks geometrically?</Highlight
    >
  </p>
  <p>
    To get to that understanding, first we have to understand, that a neural
    ...
    creating a transformation composition.
  </p>
  <Alert type="info">
    A neural network is a composition of transformations.
  </Alert>
  <p>
    To demonstrate the visual interpretation of a transformation we will utilize
    the following matrix of features with 2 features and 4 samples.
  </p>
  ...
  <p>We will start with matrix multiplications.</p>
  <Alert type="info">A matrix multiplication is a linear transformation.</Alert>
  <p>
    While there is a formal definition of linear transformations, we could use a
  ...
  <p>
    ...
    2 transformed features per sample as output.
  </p>
  ...
  <p>
    If we change the values of the identity matrix slightly, we scale the
  ...
  \end{bmatrix}
  `}</Latex
    > on the other hand scales the matrix in the y direction by a factor of 0.5.
  </p>
  <Transformation
    {inputData}
    matrix={[
  ...
  />
  <p>
    So far we have used only one diagonal of the matrix to scale the square. The
  ...
  \end{bmatrix}
  `}</Latex
    > for example, the top and the bottom lines are moved right and left respectively.
  </p>
  <Transformation
    {inputData}
    matrix={[
  ...
  \end{bmatrix}
  `}</Latex
    > on the other hand move the left and the right lines to the bottom or top respectively.
  </p>
  <Transformation
    {inputData}
    matrix={[
  ...
  />
  <p>
    We can combine scaling and sheering to achieve interesting transformations.
    The matrix
    <Latex>
      {String.raw`
      \mathbf{W}^T =
  ...
  <Alert type="info">Bias addition is a translation.</Alert>
  <p>
    A bias allows us to translate the data. That means that each point is
  ...
  <Transformation {inputData} vector={[1, 0]} />
  <p>
    The vector
    <Latex>
      {String.raw`
  ...
  <Transformation {inputData} vector={[0, 1]} />
  <p>
    A translation is not a linear transformation. If we apply a linear
  ...
  />
  <p>
    A neural network combines a liner transformation with a translation. In
    linear algebra this transformation combination is called <Highlight
      >affine transformation</Highlight
    >.
  </p>
  <p>Let's finally move to activation functions.</p>
  <Alert type="info">A nonlinear activation function squishes the data.</Alert>
  <p>
    ...
    sigmoid that we have utilized so far pushes the vectors into a 1 by 1 box.
  </p>
  <Transformation
  ...
  />
  <p>
    The ReLU activation function is even wilder. The function turns negative
    numbers into zeros and leaves positive numbers untouched. With a ReLU
    parallel lines do not necessarily stay parallel.
  </p>
  <Transformation inputData={inputData2} activation="relu" showText={false} />
  <p>
    There is a final remark we would like to make in regards with linear
    ...
    better solution in a different dimension.
  </p>
  <p>
  ...
  <span
    >Cross-Entropy: {lossOutput.toFixed(6)}<span>
      ...
      <p>
        It is not always clear how the neural network does those
        ...
        Afterwards we move the processed features back into 2d space.
      </p>
      <p>
        Modern neural networks have hundreds or thousands of dimensions and
      ...
      <Alert type="info">
        ...
        that can be linearly separated to solve a particular problem.
      </Alert>

      <p>
        Try to keep this intuition in mind while you move forward with your
        studies. It is easy to forget.
      </p>
      <div class="separator" />
    </span></span
  ></Container
