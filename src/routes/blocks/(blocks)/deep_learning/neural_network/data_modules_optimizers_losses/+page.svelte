...
<Container>
    <p>
        In the last sections we have shown a very simple implementation of a
        ...
        and scalable. This section is dedicated to those functionalities.
    </p>
    <div class="separator" />
    ...
    <p>
        ...
        way we see fit by providing the <code>Dataset</code> and the
        <code>DataLoader</code> classes.
    </p>
    <PythonCode code={code1} />
    <p>
        The <code>Dataset</code> object is the PyTorch representation of data.
        When we are dealing with real world data we subclass the
        <code>Dataset</code>
        class and overwrite the <code>__getitem__</code> and the
        <code>__len__</code>
        methods. Below we create a dataset that contains a list of numbers, the
        size of which depends on the size parameter in the
        <code>__init___</code>
        method. The <code>__getitem__</code> method implements the logic, which determines
        how the individual element of our data should be returned given only the
        index of data.
    </p>
    <PythonCode code={code2} />
    <p>
        We use the <code>ListDataset</code> to create a <code>list</code> with 100
 ...
 42</pre>
    <p>
        ...
        the index received in the <code>__getitem__</code> method. Below is a dummy
        implementation of such a Dataset.
    </p>
    <PythonCode code={code4} />
    <p>
        During the training process we only directly interact with the <code
            >DataLoader</code
        >
        object and not with the <code>Dataset</code> object. The goal of the
        <code>DataLoader</code> is to return data in batch sized pieces. Those batches
    ...
    <p>
        The approach of using the whole dataset to calculate the gradient is
        called <Highlight>batch</Highlight> gradient descent. Using the whole dataset
        ...
        all the samples in order to take just a single training step.
    </p>
    <p>
        In <Highlight>stochastic</Highlight> gradient descent we introduce some stochasticity
        ...
        the imprecise gradient could potentially push a variable from a local minimum.
    </p>

    <p>
        <Highlight>Mini-batch</Highlight> gradient descent combines the advantages
        ...
        that theoretically our training dataset can be as large as we want.
    </p>
    <p>
        The <code>DataLoader</code> takes several arguments to control the above
        described details. The <code>dataset</code> argument expects a
        <code>Dataset</code>
        object that implements the <code>__init__</code> and
        <code>__getitem__</code>
        interface. The <code>batch_size</code> parameter determines the size of
        the mini-batch. The default value is 1, which is equal to stochastic
        gradient descent. The <code>shuffle</code> parameter is a boolean value,
        that detemines if the dataset will be shuffled at the beginning of the
        iteration process. The default value is <code>False</code>.
    </p>
    <p>
        Let's generate a ListDataset with just 5 elements for demonstration
        purposes.
    </p>
    <PythonCode code={code5} />
    <p>
        We generate a DataLoader that shuffles the dataset object and returns 2
        samples at a time.
    </p>
    <PythonCode code={code6} />
    <p>
        Finally we iterate through the DataLoader and receive a batch at a time.
        Once only one object remains, a single element is returned.
    </p>
...
Batch Nr: 3 Data: tensor([2])</pre>
    <p>
        ...
        we can use the <code>drop_last</code> argument. The
        <code>drop_last</code>
        parameter removes the last batch, if it is less than
        <code>batch_size</code>. The argument defaults to False
    </p>
...
Batch Nr: 2 Data: tensor([3, 4])
    ...
    <p>
        Each sample in the dataset is typically used several times in the
        training process. Each iteration over the whole dataset is called an <Highlight
            >epoch</Highlight
        >.
    </p>
    <Alert type="info">
        An epoch is the time period, in which all the samples in the dataset
        have been iterated over and used for gradient calculations.
    </Alert>
    <p>
        If we want to use several epochs in a training loop, all we have to do
        is to include an additional outer loop.
    </p>
...
Epoch Nr: 2 Batch Nr: 2 Data: tensor([3, 1])
    ...
    <p>
        ...
        The <code>num_workers</code> parameter determines the number of workers,
        that get the data in parallel. The default is 0, which means that only the
        main process is used.
    </p>
    <PythonCode code={code11} />
    <p>
        We won't notice the speed difference using such a simple example, but
        the speedup with large datasets might be noticable.
    </p>
    <p>
        There are more parameters, that the <code>DataLoader</code> class
    ...
    <h2>Training Loop</h2>
    <p>
        The training loop that we implemented when we solved our circular
        ...
        be glad that we are able to utilize a more efficient training approach.
    </p>
    <PythonCode code={code12} />
    <p>
        This time around we explicitly set some parameters as constants. This
        time around we use a much higher number of samples and neurons, to
        demonstrate that PyTorch is able to handle those.
    </p>
    <PythonCode code={code13} />
    <p>
        We create a simple classification dataset with sklearn and construct a <code
            >Dataset</code
        > object.
    </p>
    ...
    <p>
        ...
        another we will calculate our forward pass using a single call to the <code
            >model</code
        ...
        a certain class. The <code>criterion</code> is essentially a loss
        ...
        when we call <code>optimizer.step</code> and clears all the gradients
        when we call <code>optimizer.zero_grad()</code>.
    </p>
    <PythonCode code={code16} />
    <p>
        In order to make our calculations more modular, we will create a <code
            >Module</code
        >
        ...
        class <code>Module</code>. This module initializes a weight matrix and a
        bias vector. For easier access at a later point we create an attribute
        <code>parameters</code>, which is just a list holding the weights and
        biases. We also implement the <code>__call__</code> method, which contains
        the logic for the forward pass.
    </p>
    <PythonCode code={code17} />
    <p>
        Our model needs an activation function, so we implement a sigmoid
        function.
    </p>
    <PythonCode code={code18} />
    <p>
        The <code>Model</code> class is the abstraction of the neural network.
        We will need three fully connected layers, so the model initializes
        three linear modules. In the <code>__call__</code> method we implement
        forward pass of the neural network. So when we call
        <code>model(features)</code>, the features are processed by the neural
        network, until the last layer is reached. Additionally we implement the
        <code>parameters</code> method, which returns the full list of the parameters
        of the model.
    </p>
    <PythonCode code={code19} />
    <p>
        Below we test the forward pass with random numbers. Applying the forward
        pass of a predefined model should feel more intuitive than our previous
        implementations.
    </p>
    <PythonCode code={code20} />
    <p>
        ...
        learning rate (alpha) and the parameters of the model. When we call <code
            >step()</code
        >
        we loop over all parameters and apply gradient descent and when we call
        <code>zero_grad()</code> we clear all the gradients. Notice that the optimizer
        logic works independent of the exact architecture of the model, making the
        code more managable.
    </p>
    <PythonCode code={code21} />
    <p>
        Finally we implement the loss function. Once again the calculation of
...
Epoch: 10 Loss: 0.1082562804222107
    ...
    <p>
        ...
        we implemented above, out of the box. The PyTorch module <code
            >torch.nn</code
        > contains most of the classes and functions, that we will require.
    </p>
    <PythonCode code={code25} />
    <p>
        When we write custom PyTorch modules we need to subclass <code
            >nn.Module</code
        >. We need to putall trainable parameters into the
        <code>nn.parameter.Parameter()</code>
        ...
        <code>__call__</code>
        as we did before, we define the <code>forward</code> method. PyTorch
        calls <code>forward</code> automatically, when we call the module
        ...
        <code>module.forward(features)</code>
        use <code>module(features)</code>.
    </p>
    <PythonCode code={code26} />
    <p>
        ...
        use the above defined <code>Module</code> class in the
        <code>Model</code> module. In later chapter we will see how we can create
        blocks of arbitrary complexity using this simple approach.
    </p>
    <PythonCode code={code27} />
    <p>
        PyTorch obviously provides loss functions and optimizers. We will use <code
            >BCELoss</code
        >, which calculates the binary cross-entropy loss. Optimizers are
        located in <code>torch.optim</code>. For now we will use stochastic
        gradient descent, but there are many more optimizers that we will
        encounter soon.
    </p>
    <PythonCode code={code28} />
    <PythonCode code={code29} />
    <pre class="text-sm">
...
Epoch: 10 Loss: 0.10799615830183029
    ...
    <p>
        ...
        transformation layer is a common procedure, therefore you should use <code
            >nn.Linear</code
        > instead of implementing your solutions from scratch.
    </p>
    <PythonCode code={code30} />
    <PythonCode code={code31} />
    <PythonCode code={code32} />
    <pre class="text-sm">
...
Epoch: 10 Loss: 0.11287659406661987
    ...
    <p>
        ...
        pack all modules and activations into a <code>nn.Sequential</code> object.
        When we call that object, the components will be executed in a sequential
        order.
    </p>
    <PythonCode code={code33} />
    <PythonCode code={code34} />
    <PythonCode code={code35} />
    <pre class="text-sm">
