...
<Container>
  ...
  <p>
    ...
    networks to learn sequence models. Yet we still face a problem that we need
    to solve, before we can train those models on text.
  </p>
  <Alert type="info">
    ...
    compatible for training and inference, it needs to be transformed into a
    numerical representation. In other words text needs to be vectorized.
  </Alert>
  <p>
  ...
  <p>
    ...
    individual words. Tokenization can be a daunting task, so we will stick to
    the basics here.
  </p>
  <!-- split sentences -->
  <div class="tokens">
  ...
  <p>
    ...
    punctuation and turing letters into their lower case counterparts.
  </p>
  <!-- standardized words -->
  <div class="tokens">
    {#each sentence1.split(" ") as word}
  ...
  <p>
    ...
    sequence. For that purpose we use padding ({"<pad>"}). The token for unknown
    words {"<unk>"} is used for words that are outside of the vocabulary. This happens
    for example if the vocabulary that is built using the training dataset does not
    contain some words from the testing dataset. Additionally we often limit the
    size of the vocabulary in order to save computational power. In our example we
    assume that roman numerals are extremely rare and replace them by the special
    token.
  </p>
  ...
  <p>
    To turn each index into a vector we use one-hot encoding. These vectors are
    as long as there are tokens in the vocabulary. For the most part the vector
    consists of zeros, but at the correct index the value is 1. For our
    vocabulary of size 13, we have access to 13 one-hot vectors.
  </p>
  <div class="tokens">
    <div>
      {#each vocabulary as word, idx}
  ...
  <p>
    Our second sentence for example would correspond to a sequence of the
  ...
  <p>
    Below we exemplify how those embeddings might look like. We turn
    13-dimensional sparse vectors into 4-dimensional dense vectors.
  </p>
  <div class="tokens">
    <div>
      {#each vocabulary as word, idx}
  ...
  <p>
    ...
    want to turn it into a 2-dimensional word embedding, we define the word
    embedding matrix as trainable weights of the corresponding size and multiply
    the two.
  </p>
  <Latex
    >{String.raw`
  ...
  <p>
    ...
    vector, that can be used as input into our sequence model. This operation
    will be tracked by the autograd package and those weights will update over
    the time, optimizing the embedding representation.
  </p>
  <p>
    In practice all major frameworks have a dedicated embedding layer, that does
    this operation via a lookup. This is just a much more efficient
    representation, but the results should be the same. We will look at what
    that means when we start implementing language models.
  </p>
  <div class="separator" />
</Container>
